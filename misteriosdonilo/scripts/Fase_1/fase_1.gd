...
func iniciar_jogo():
	...
	# ⭐ DESABILITADO: Não criar cards dinamicamente - usar apenas os cards da cena
...
func criar_cards_dinamicamente():
	...
	for i in range(valores_cards.size()):
		...
		if card_instance is CardResposta:
			...
			# ⭐ CORREÇÃO: Posicionar cards verticalmente à direita, alinhados próximo às áreas
			# As áreas estão em Y: 142, 296, 427. Vamos posicionar os cards próximos a essa região
			var pos_y_base = 150  # Base Y próximo à primeira área
			var espacamento = 80  # Espaçamento entre cards
			card.position = Vector2(0, pos_y_base + i * espacamento)
