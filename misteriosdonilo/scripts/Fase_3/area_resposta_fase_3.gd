extends Area2D
class_name AreaResposta_3

signal resposta_recebida(valor: int, correto_para_esta_area: bool)
signal card_entrou_na_area(area: AreaResposta_3, card: CardResposta_3)
...
func _ready():
	...
	if resultado_esperado == 0 and expressao.is_empty():
		push_warning("AreaResposta_3 não foi configurada corretamente - use a função configurar()")
...
func _on_area_entered(area: Area2D):
	...
	# Verificar se é um CardResposta_3
	if area is CardResposta_3:
		var card: CardResposta_3 = area
		
		if not card.has_method("get_valor"):
			push_error("CardResposta_3 não possui método get_valor()")
			return
...
func _processar_resposta(_valor_card: int, _card: CardResposta_3):
	...
	# ⭐⭐ IMPORTANTE: Emitir sinal SEMPRE para que Fase_3.gd saiba o resultado
...
func get_info() -> String:
	return "AreaResposta_3: %s = %d (Correto: %s)" % [expressao, resultado_esperado, tem_card_correto]
